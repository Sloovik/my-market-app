...
public class ItemService {
    ...
    public List<List<ItemDto>> getPagedItems(String search, String sort, int pageNumber, int pageSize, CartService cartService) {
                ...
                .mapToObj(rowIndex -> {
                    ...
                    return row.stream()
                            .map(item -> new ItemDto(
                                    ...
                                    item.getImgPath(), item.getPrice(),
                                    cartService.getCount(item.getId(), null)))
    ...
    public PagingDto getPaging(String search, String sort, int pageNumber, int pageSize) {
        long totalItems = search == null || search.trim().isEmpty() ?
                itemRepository.count() :
                itemRepository.findBySearch("%" + search.trim() + "%", PageRequest.of(0, 1)).getTotalElements();

        int totalPages = (int) Math.ceil((double) totalItems / pageSize);
        return new PagingDto(pageSize, pageNumber, pageNumber > 1, pageNumber < totalPages);
    }

    public ItemDto getItemDto(Long id, CartService cartService) {
        Item item = itemRepository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Товар с id=" + id + " не найден"));
        return new ItemDto(
                item.getId(), item.getTitle(), item.getDescription(),
                item.getImgPath(), item.getPrice(),
                cartService.getCount(id, null)
        );
    }
